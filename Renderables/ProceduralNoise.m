classdef ProceduralNoise < Renderable
    ...
    methods
        ...
        function initialize(obj)
            ...
            motion_shift_frames = round(obj.getIFI() * obj.motion_shift);
            ...
            spatial_filter(1:center,center+1:obj.size) = filter_centered(center+1:obj.,1:center);
...
% initalize
obj.noise_image = zeros(image_size,image_size,num_frames);
obj.n_frames = 2 * (1/obj.getIFI());
...
for i = 1:obj.n_frames
    ...
    if rem(i,motion_shift_frames)==1
        polarity_mat = round(rand(image_size)*2-1).*rand(image_size)*pi;
    end
    ...
    obj.textures(i) = Screen('MakeTexture', obj.getWindow(), norm_image);
end
end
        ...
        function draw(obj, t_close)
            ...
            while obj.renderer.getTime() < t_close
                ...
                Screen('DrawTexture',obj.getWindow(), obj.textures(frame_idx), obj.getRect(obj.size), obj.getRect(obj.size), obj.ori, [], [], [], [], obj.rotate_mode, [phase, obj.spat_freq, obj.amplitude, 0]); % subsample
